...
namespace RothschildHouse.Application.Features.Transactions.Commands;
...
public class ProcessTransactionCommandHandler : IRequestHandler<ProcessTransactionCommand, ProcessTransactionResponse>
{
    ...
    private readonly ICityBankPaymentServicesClient _cityBankPaymentServices;
    private readonly SearchEngineClient _searchEngine;

    public ProcessTransactionCommandHandler(IRothschildHouseDbContext dbContext, ICityBankPaymentServicesClient cityBankPaymentServices, SearchEngineClient searchEngine)
    {
        _dbContext = dbContext;
        _cityBankPaymentServices = cityBankPaymentServices;
    ...
    public async Task<ProcessTransactionResponse> Handle(ProcessTransactionCommand request, CancellationToken cancellationToken)
    {
        ...
        var paymentGatewayResponse = await _cityBankPaymentServices.ProcessPaymentAsync(paymentGatewayRequest);
        ...
        if (paymentGatewayResponse.Successed)
        {
            ...
            await _searchEngine.IndexSaleAsync(new IndexSaleRequest
            {
                TxnId = txn.Id,
                TxnGuid = txn.Guid,
